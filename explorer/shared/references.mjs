const tokenPattern = /^cadproto:v2:([a-f0-9]{64})\/([A-Za-z0-9._~!%*'()-]+)\/(part|[fe][1-9][0-9]*)$/;
const oneLine = (text) => {
...
export function parseReference(text) {
  ...
  return { topologyRevision: match[1], nodeId, faceId: match[3].startsWith("f") ? match[3] : null, edgeId: match[3].startsWith("e") ? match[3] : null };
}

export function referenceEntity(model, nodeId, faceId = null, edgeId = null) {
  ...
  const edge = edgeId === null ? null : part.edges?.find((item) => item.id === edgeId);
  if (edgeId !== null && !edge) throw new Error("Reference edge is not present on this occurrence");
  return { node, part, face, edge };
}

export function createReference(model, nodeId, faceId = null, edgeId = null) {
  if (!/^[a-f0-9]{64}$/.test(model.topologyRevision)) throw new Error("Model topology identity is unavailable");
  referenceEntity(model, nodeId, faceId, edgeId);
  const reference = `cadproto:v2:${model.topologyRevision}/${encodeURIComponent(nodeId)}/${faceId ?? edgeId ?? "part"}`;
  parseReference(reference);
  return reference;
}

export function formatSelection(model, nodeId, faceId = null, edgeId = null) {
  const { node, face, edge } = referenceEntity(model, nodeId, faceId, edgeId);
  const entity = face ? `Face ${face.id} (${face.surfaceType}, ${face.area.toFixed(2)} mm^2)`
    : edge ? `Edge ${edge.id} (${edge.curveType}, ${edge.length.toFixed(2)} mm)` : "Whole part";
  return `CAD Explorer | ${oneLine(model.source.name)} | ${oneLine(node.label)} | ${entity}\n${createReference(model, nodeId, faceId, edgeId)}`;
}
